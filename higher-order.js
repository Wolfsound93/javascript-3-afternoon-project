...
//Code Here
let totalPopulation = populations.reduce((val, currentVal) => val + currentVal);
...
let orderTotals = orders.map(function(val) {
  let total = val.price * val.tax + val.price;
  // newArr.push(total);
  // return newArr;
  return total;
});
...
let bobsTotal = purchases.reduce(function(val, current) {
  console.log(current);
